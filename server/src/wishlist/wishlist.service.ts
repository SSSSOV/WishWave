...
export class WishlistService {
  ...
  async searchFriendLists(requestId: number, friendId: number, nameSearch: string): Promise<WishList[]> {
    ...
    const friendship = await this.friendRepository.findOne({
      where: {
        friendstatusId: acceptedId,
        [SqOp.or]: [
          { sender: requestId, recipient: friendId },
          { sender: friendId, recipient: requestId },
        ],
      },
    })
    if (!friendship) {
    ...
    const lists = await this.wishListRepository.findAll({
      where: { userId: friendId, name: { [SqOp.iLike]: `%${nameSearch}%` } },
      include: [{ model: AccessLevel, as: "accesslevel" }],
    })
    return lists
  }
        ...
        { model: User, as: "user", attributes: ["id", "login", "fullname", "email", "image"] },
        {
          ...
          include: [
            { model: WishStatus, attributes: ["id", "name"] },
            {
              model: User,
              as: "bookedByUser",
              attributes: ["id", "fullname", "login", "image"],
            },
          ],
        },
      ],
  ...
  async getAccessibleWishes(viewerId: number, ownerId: number): Promise<WishList[]> {
    const allLists = await this.wishListRepository.findAll({
      where: { userId: ownerId },
      attributes: ["id", "shareToken", "userId"],
      include: [{ model: AccessLevel, as: "accesslevel", attributes: ["name"] }],
    })
    ...
    for (const wl of allLists) {
      ...
      if (ownerId === viewerId) {
        ...
        const friendship = await this.friendRepository.findOne({
          where: {
            friendstatusId: acceptedStatusId,
            [SqOp.or]: [
              { sender: viewerId, recipient: ownerId },
              { sender: ownerId, recipient: viewerId },
            ],
          },
        })
    ...
    return this.wishListRepository.findAll({
      where: { id: accessibleListIds },
      attributes: ["id", "name", "accesslevelId", "description", "eventDate", "shareToken"],
      include: [{ model: User, as: "user", attributes: ["id", "fullname", "login", "image"] }],
    })
  }

  async findByIdBare(wishlistId: number): Promise<WishList | null> {
    return this.wishListRepository.findByPk(wishlistId, {
      include: [
        { model: AccessLevel, as: "accesslevel", attributes: ["id", "name"] },
  ...
  async getAllWishlistAdm(page: number, perPage: number) {
    ...
    const options: FindAndCountOptions = {
      attributes: ["id", "name", "description", "eventDate", "shareToken", "userId", "accesslevelId", "createdAt", "updatedAt"],
      limit: perPage,
      offset,
      order: [["id", "ASC"]],
    }
